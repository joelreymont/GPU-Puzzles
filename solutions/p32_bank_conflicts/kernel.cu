...
__global__ void TransposeSwizzle(const float* in, float* out, int rows,
  ...
  /// BEGIN CODE HERE (approx 8 lines) ///
  for (int j = 0; j < TILE; j += BLOCK_ROWS) {
    const int r = threadIdx.y + j;  // tile row this thread writes
    if (x < cols && y + j < rows)
      tile[r][threadIdx.x ^ r] = in[(y + j) * cols + x];
  }
  __syncthreads();
  for (int j = 0; j < TILE; j += BLOCK_ROWS) {
    const int b = threadIdx.y + j;  // tile column this thread reads
    if (tx < rows && ty + j < cols)
      out[(ty + j) * rows + tx] = tile[threadIdx.x][b ^ threadIdx.x];
  }
  /// END CODE HERE ///
}
