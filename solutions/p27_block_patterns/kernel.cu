...
__global__ void BlockSumTwoStage(const float* a, float* out, int n) {
  ...
  __syncthreads();
  if (warp == 0) {
    float t = (lane < WARPS) ? partials[lane] : 0.0f;
    for (int d = WARPS / 2; d > 0; d >>= 1) {
...
__global__ void BlockSumCUB(const float* a, float* out, int n) {
  ...
  float total = BlockReduce(tmp).Sum(v);
  if (threadIdx.x == 0) {
    atomicAdd(out, total);
  }
  /// END CODE HERE ///
...
__global__ void BlockScanCUB(const float* a, float* out, int n) {
  ...
  /// BEGIN CODE HERE (approx 4 lines) ///
  float s = 0.0f;
  BlockScan(tmp).InclusiveSum(v, s);
  if (i < n) out[i] = s;
  /// END CODE HERE ///
}
...
__global__ void HistogramShared(const float* a, int* hist, int nbins, int n) {
  ...
  for (int k = threadIdx.x; k < nbins; k += blockDim.x) bins[k] = 0;
  __syncthreads();
  if (i < n) {
    float v = a[i];
