...
__global__ void PolyOne(const float* a, float* out, int n) {
  ...
  const float x = (i < n) ? a[i] : 0.0f;  // value guard, not an early return
  float s = coef(TERMS - 1);
#pragma unroll
  for (int k = TERMS - 2; k >= 0; k--) s = fmaf(s, x, coef(k));
  if (i < n) out[i] = s;
  /// END CODE HERE ///
}
...
__global__ void PolyFour(const float* a, float* out, int n) {
  ...
  float x[ELEMS], s[ELEMS];
#pragma unroll
  for (int t = 0; t < ELEMS; t++) {
    const int j = i + t * stride;
    x[t] = (j < n) ? a[j] : 0.0f;  // value guard: the tail is short
    s[t] = coef(TERMS - 1);
  }
#pragma unroll
  for (int k = TERMS - 2; k >= 0; k--)
#pragma unroll
    for (int t = 0; t < ELEMS; t++) s[t] = fmaf(s[t], x[t], coef(k));
#pragma unroll
  for (int t = 0; t < ELEMS; t++) {
    const int j = i + t * stride;
    if (j < n) out[j] = s[t];
...
__global__ void __launch_bounds__(256, 6)
  ...
  stage[threadIdx.x] = (i < n) ? a[i] : 0.0f;
  __syncthreads();
  const float x = stage[threadIdx.x];
  float s = coef(TERMS - 1);
#pragma unroll
  for (int k = TERMS - 2; k >= 0; k--) s = fmaf(s, x, coef(k));
  if (i < n) out[i] = s;
