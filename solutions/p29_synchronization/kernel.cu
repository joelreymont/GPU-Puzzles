...
__global__ void StencilSyncthreads(const float* a, float* out, int n) {
  ...
  // Cooperative load: SH = 264 slots over 256 threads, so the first 8 threads
  // take a second one. A slot whose global index is off the array clamps to
  // the nearest real element, which is the same boundary rule the stencil uses.
  for (int j = threadIdx.x; j < SH; j += TPB)
    sh[0][j] = a[min(max(base - HALO + j, 0), n - 1)];
  __syncthreads();

  int p = 0;  // read sh[p], write sh[p^1]
  for (int k = 0; k < T; k++) {
    for (int j = threadIdx.x; j < SH; j += TPB)
      sh[p ^ 1][j] = 0.25f * sh[p][max(j - 1, lo)] + 0.5f * sh[p][j] +
                     0.25f * sh[p][min(j + 1, hi)];
    // One barrier per step, covering both hazards at once: it publishes this
    // step's writes to sh[p^1] before anyone reads them (RAW), and it proves
    // every read of sh[p] has happened before the next step overwrites sh[p]
    // (WAR). Single-buffered code would need two barriers here.
    __syncthreads();
    p ^= 1;
  }

  const int i = base + threadIdx.x;
  if (i < n) out[i] = sh[p][HALO + threadIdx.x];
  /// END CODE HERE ///
}
...
__global__ void StencilBarrierArriveWait(const float* a, float* out, int n) {
  ...
  for (int j = threadIdx.x; j < SH; j += TPB)
    sh[0][j] = a[min(max(base - HALO + j, 0), n - 1)];
  bar.arrive_and_wait();  // nothing to overlap here: the next read needs it all

  float* cur = sh[0];
  float* nxt = sh[1];
  for (int k = 0; k < T; k++) {
    for (int j = threadIdx.x; j < SH; j += TPB)
      nxt[j] = 0.25f * cur[max(j - 1, lo)] + 0.5f * cur[j] +
               0.25f * cur[min(j + 1, hi)];
    auto tok = bar.arrive();  // release: my writes to nxt are published
    float* tmp = cur;         // independent work: rotate the buffer roles for
    cur = nxt;                // the next step. Registers only -- it touches no
    nxt = tmp;                // shared memory, so the barrier need not order it
    bar.wait(cuda::std::move(tok));  // acquire: every thread's writes are mine
  }

  const int i = base + threadIdx.x;
  if (i < n) out[i] = cur[HALO + threadIdx.x];
  /// END CODE HERE ///
}
